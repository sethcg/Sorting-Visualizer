...
TEST(InsertionSort_Test, GetStepCount_ListSize) {
    // ARRANGE
    ...
    // ASSERT
    EXPECT_EQ(stepCount, LIST_SIZE);
}

TEST(InsertionSort_Test, GetStepCount_GreaterThanOne) {
    // ARRANGE
    std::unique_ptr<Rectangle[]> itemPtr = std::make_unique<Rectangle[]>(LIST_SIZE);
...
TEST(InsertionSort_Test, SetSequence_CorrectOrder) {
    // ARRANGE
    std::unique_ptr<Rectangle[]> itemPtr = std::make_unique<Rectangle[]>(LIST_SIZE);
